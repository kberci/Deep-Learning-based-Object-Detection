...
def get_object_name(class_id, model='mobilenet'):
    with open('models/coco_label_map.pbtxt', 'r') as f:
        # if model == 'yolo4':
...
MODEL_NAME = '.'
GRAPH_NAME = 'models/mobiledet_edgetpu_hardhat.tflite'
min_conf_threshold = .6
resW, resH = 640, 480
imW, imH = int(resW), int(resH)
use_TPU = True

if 'hardhat' in GRAPH_NAME:
    LABELMAP_NAME = 'models/hardhat_label_map.pbtxt'
else:
    LABELMAP_NAME = 'models/coco_label_map.pbtxt'
...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            # Draw label
            object_name = get_object_name(int(classes[i]))[0]  # Look up object name from "labels" array using class index
